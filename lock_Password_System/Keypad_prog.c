...
u8 KEYPAD_GetPressedKey(void)
{
	...
	{
		/*put on each rows 0*/
		SetPinValue(KEYPAD_ROWSPortArray[ROWSCounter], KEYPAD_ROWSPinArray[ROWSCounter],0);
		/*check every col*/
		for(COLSCounter=0; (COLSCounter<KEYPAD_COLMAXNUM)&&( flag==0); COLSCounter++)
		{
			if(GetPinValue(KEYPAD_COLSPortArray[COLSCounter],KEYPAD_COLSPinArray[COLSCounter])==0)
			{
		...
		SetPinValue(KEYPAD_ROWSPortArray[ROWSCounter], KEYPAD_ROWSPinArray[ROWSCounter],1);
